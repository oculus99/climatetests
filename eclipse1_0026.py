...
"""
Planeetan energia-tasapainomalli - täysi versio (ω-skaalattu)
- Fraktaalipinta (meri/manner) 3D-pallokoordinaateissa (pnoise3)
- Ilmakehän solut: Hadley + Ferrel + Polar (omega-skaalaus)
- Subtrooppinen kuivuus: sijainti riippuu ω:sta (30° * ω^-0.3)
- ITCZ-piikin leveys riippuu ω:sta (10° / ω^0.3)
- NAPAKORJAUS 1: Pysyvä napajää albedossa (+0.25, 25°)
- NAPAKORJAUS 2: Sulamisenergia lämpökapasiteetissa (×6, 25°)
- NAPAKORJAUS 3: Inversio estää sekoituksen (20°)
- Merivirrat, kasvihuone F_perus=185
- Pilvisyys: 0.55 meri / 0.40 manner + ITCZ + subtrooppinen kuivuus
...
def albedo_pinnan_mukaan(onko_manner, korkeus_m, T, pilvisyys=None, LAT=None):
    ...
    if LAT is not None:
        ...
        napa_paino = np.exp(-((lat_abs_deg - 90) / 25)**2)
        albedo = albedo + 0.25 * napa_paino
...
def ilmakehan_kuljetus(T, onko_manner, omega_suhteessa_maahan, vahvuus=0.08,
    ...
    kappa_merid += 0.10 * np.exp(-(lat_abs / 20)**2)
    kappa_merid += 0.40 * np.exp(-((lat_abs - 30) / 25)**2)
    kappa_merid += 0.10 * np.exp(-((lat_abs - 75) / 15)**2)

    napa_esto = np.exp(-((lat_abs - 90) / 20)**2)
    kappa_merid = kappa_merid * (1 - 0.7 * napa_esto)
...
# ============================================================
# 6. KOSTEUS, PILVET JA SADE (ω-skaalattu)
# ============================================================
def kosteus_pilvet_sade(T, I_abs, onko_manner, korkeus_m, p_atm, LAT,
    ...
    # ============================================
    # ω-SKAALATTU SUBTROOPPINEN KUIVUUS
    # ============================================
    omega_clip = max(omega_suhteessa_maahan, 0.1)
    subtrooppinen_lat = 30.0 * (omega_clip ** -0.3)
    subtrooppinen_lat = np.clip(subtrooppinen_lat, 15, 55)
    lat_abs_deg = np.abs(np.rad2deg(LAT))
    subtrooppinen = np.exp(-((lat_abs_deg - subtrooppinen_lat) / 10)**2)

    K = K * (1 - 0.6 * subtrooppinen)
    ...
    # ITCZ-piikin leveys ω:n mukaan
    itcz_leveys = 10.0 / (omega_clip ** 0.3)
    itcz_leveys = np.clip(itcz_leveys, 5, 20)
    itcz_paino = np.exp(-(lat_abs_deg / itcz_leveys)**2)
    pilvisyys = pilvisyys + 0.15 * itcz_paino * T_paino

    # Subtrooppinen pilvettömyys
    pilvisyys = pilvisyys * (1 - 0.5 * subtrooppinen)
...
# ============================================================
# 7. LÄMPÖKAPASITEETTI + NAPAKORJAUS 2
# ============================================================
def lampokapasiteetti(onko_manner, korkeus_m, LAT,
                      meren_syvyys=MEREN_SYVYYS_M):
    C_meri = 4.0e6 * meren_syvyys
    C_manner = 2.0e6 * 2.0
    C = np.where(onko_manner, C_manner, C_meri)
...
def aja_malli(
    ...
    C = lampokapasiteetti(onko_manner, korkeus_m, LAT)
    dt = 30 * 24 * 3600
    ...
    for kk in range(kuukausia_yhteensa):
        ...
        if debug and kk == 0:
            ...
            print(f"  ω = {omega_suhteessa_maahan:.2f}")
            print(f"  Subtrooppinen lat: {30.0 * omega_clip**-0.3:.1f}°")
            print(f"  ITCZ leveys: {10.0 / omega_clip**0.3:.1f}°")

        for _ in range(5):
            albedo = albedo_pinnan_mukaan(onko_manner, korkeus_m, T, pilvisyys, LAT)
        ...
        sade_kk, pilvisyys_uusi, _ = kosteus_pilvet_sade(
            ...
            omega_suhteessa_maahan=omega_suhteessa_maahan,
...
if __name__ == "__main__":
    print("=== Maan kaltainen planeetta - ω-skaalattu subtrooppinen ===\n")
    tulos = aja_malli(
    ...
    piirra_tulokset(tulos, otsikko="Maan kaltainen planeetta - ω-skaalattu subtrooppinen")
