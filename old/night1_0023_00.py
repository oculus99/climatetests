...
def laske_perus_sade(lat_grid_deg,
                     ilmastollinen_paivantaasaaja,
                     tropiikin_max_sade=200.0,
                     lauhkean_sade=60.0,
                     tropiikin_leveys=12.0,
                     siirtymaleveys=20.0):

    # Etäisyys ilmastollisesta päiväntasaajasta
    etaisyys = cp.abs(lat_grid_deg - ilmastollinen_paivantaasaaja)

    # Trooppisen sadevyöhykkeen voimakkuus.
    # exp-muotoinen profiili tekee sateen maksimista melko
    # selväpiirteisen mutta ei täysin jyrkkää reunaa.
    tropiikki = cp.exp(
        -(etaisyys / tropiikin_leveys) ** 2
    )

    # Lauhkean vyöhykkeen perussade.
    # Pidetään tämä lähes kaikkialla taustalla.
    perussade = lauhkean_sade

    # Tropiikin sade lisääntyy ilmastollisen päiväntasaajan lähellä.
    sade = perussade + (
        tropiikin_max_sade - lauhkean_sade
    ) * tropiikki

    # Vähennetään sadetta vähitellen kohti napoja.
    # 0° -> 1, 90° -> 0.
    napakerroin = cp.cos(cp.deg2rad(cp.abs(lat_grid_deg))) ** 0.5
...
def laske_ilmasto_gpu(korkeus_syvyyskartta, planet_mass_me=1, planet_radius_re=1.0, planet_tilt=23.44, planet_ecc=0.013, planet_mvelp_deg=101.2, tmax_default=57.0, global_mousture_coeff=1.0):
    ...
    R
    * dlon_rad
    * np.cos(np.deg2rad(lat_grid_deg))
    )
    korkeuskartta=np.copy(korkeus_syvyyskartta)
    korkeuskartta=np.where(korkeuskartta<0,0,korkeuskartta)
    syvyyskartta=np.copy(korkeus_syvyyskartta)
    syvyyskartta=np.where(syvyyskartta>=0,0,syvyyskartta)
    korkeus_gradientti_x = np.gradient(korkeuskartta, axis=1)
    korkeus_gradientti_y = np.gradient(korkeuskartta, axis=0)
    syvyys_gradientti_x = np.gradient(syvyyskartta, axis=1)
    syvyys_gradientti_y = np.gradient(syvyyskartta, axis=0)

    maa_maski=np.copy(korkeuskartta)
    maa_maski=np.where(maa_maski>0,1,0)
    
    meri_maski=np.copy(korkeuskartta)
    meri_maski=np.where(meri_maski>0,0,1)   
    meri_bin=meri_maski.astype(bool)
    etaisyys_meresta_km=laske_etaisyys_rannikosta_gpu(
    maa_maski,
    maapallon_sade_km=6371.0088*planet_radius_re,
    )



    kuukausi_lämpötilat_gpu = np.zeros((12, korkeus, leveys))
    kuukausi_sateet_gpu = np.zeros((12, korkeus, leveys))
    kuukausi_tuuli_suunta_gpu= np.zeros((12, korkeus, leveys))
    kuukausi_tuuli_voima_gpu= np.zeros((12, korkeus, leveys))
    kuukausi_merivirta_x_gpu= np.zeros((12, korkeus, leveys))
    kuukausi_merivirta_y_gpu= np.zeros((12, korkeus, leveys))

    for kk in range(0,11):
        peruslampotulos=laske_auringon_peruslampotila(cp.asarray(lat_grid_deg),kk,planet_tilt,planet_ecc,planet_mvelp_deg,tmax_default)
        kuukausi_lämpötilat_gpu[kk]=peruslampotulos[0].get()
        perustuulitulos=laske_tuulet(cp.asarray(lat_grid_deg),kk,tilt_planet_axis,)
        tuuli_x=perustuulitulos[0].get()
        tuuli_y=perustuulitulos[1].get()
        tuuli_x_yksikko=perustuulitulos[2].get()
        tuuli_y_yksikko=perustuulitulos[3].get()      
        tuuli_suunta=perustuulitulos[4].get()
        tuuli_voima=perustuulitulos[5].get()
        kuukausi_tuuli_suunta_gpu[kk]=tuuli_suunta
        kuukausi_tuuli_voima_gpu[kk]=tuuli_voima        
        ilmastollinen_paivantaasaaja=perustuulitulos[6]
        orografiatulos=laske_orografia(
        cp.asarray(korkeuskartta),
        cp.asarray(korkeus_gradientti_x),
        cp.asarray(korkeus_gradientti_y),
        cp.asarray(tuuli_x_yksikko),
        cp.asarray(tuuli_y_yksikko),
        cp.asarray(tuuli_voima),)
        ...
        merivirrat_tulos=laske_merivirrat(cp.asarray(lat_grid_deg),cp.asarray(syvyyskartta),cp.asarray(meri_bin),cp.asarray(etaisyys_meresta_km),cp.asarray(tuuli_x_yksikko),cp.asarray(tuuli_y_yksikko),cp.asarray(tuuli_voima),kk,ilmastollinen_paivantaasaaja,)
        
        virta_x=merivirrat_tulos[0].get()
        virta_y=merivirrat_tulos[1].get()
        virta_voima=merivirrat_tulos[2]
        virta_suunta=merivirrat_tulos[3]
        merivesi_anomalia=merivirrat_tulos[4]
        rannikko_merivirta_anomalia=merivirrat_tulos[4]
        kuukausi_merivirta_x_gpu[kk]=virta_x
        kuukausi_merivirta_y_gpu[kk]=virta_y
        perus_sade_tulos=laske_perus_sade(cp.asarray(lat_grid_deg),ilmastollinen_paivantaasaaja,tropiikin_max_sade=200.0,lauhkean_sade=60.0,tropiikin_leveys=12.0,siirtymaleveys=20.0)
        ilmastollinen_paivantaasaaja = perustuulitulos[6]

        perussade = laske_perus_sade(
        cp.asarray(lat_grid_deg),
        ilmastollinen_paivantaasaaja
        )

        kuukausi_sateet_gpu[kk] = perussade.get()        
        tarkennetut_sadevyohykeet_tulos=tarkenna_sadevyohykkeet(
        perussade,
        cp.asarray(lat_grid_deg),
        ilmastollinen_paivantaasaaja,
        )
        kuukausi_sateet_gpu[kk] =tarkennetut_sadevyohykeet_tulos.get() 
        #3 distance to costa only
        #rannikkokerroin_tulos=laske_rannikkokerroin(
        #cp.asarray(etaisyys_meresta_km),
        #kosteus_max=1.5,
        #vaikutusmatka_km=500.0,
        #)
        #kuukausi_sateet_gpu[kk] =kuukausi_sateet_gpu[kk]*rannikkokerroin_tulos.get()
        # Kaikkialla tuuli lännestä itään
        #tuuli_x_test = cp.ones(meri_bin.shape,dtype=cp.float32)

        #tuuli_y_test = cp.zeros(
        #meri_bin.shape,
        #dtype=cp.float32
        #)
        ...
        #print(
        #"tuuli x:",
        #float(cp.min(tuuli_x_yksikko)),
        #float(cp.max(tuuli_x_yksikko))
        #)

        #print(
        #"tuuli y:",
        #float(cp.min(tuuli_y_yksikko)),
        #float(cp.max(tuuli_y_yksikko))
        #)
        tuuli_kulma = tuuli_suunta
        #print(
        #"kulma min:",
        #float(cp.min(tuuli_kulma)),
        #"max:",
        #float(cp.max(tuuli_kulma))
        #)
        ...
        tuulietaisyys_meresta_km=tuulietaisyys_tulos.get()
        
        #plt.imshow(tuulietaisyys, vmin=0, vmax=500, cmap="RdBu")
        #plt.show()
        #quit(-1)
        merelta_tuleva_kosteus_tulos=laske_merelta_kulkeva_kosteus(
        cp.asarray(tuulietaisyys_meresta_km),
        kosteus_vaikutusmatka_km=700.0,
        kosteus_max=1.5,)
        kuukausi_sateet_gpu[kk] =kuukausi_sateet_gpu[kk]*merelta_tuleva_kosteus_tulos.get()
        sade_orografia_vaikutuskerroin_tulos=laske_orografiakerroin(
        cp.asarray(korkeuskartta),cp.asarray(orografinen_pakote),korkeus_kerroin=0.00005,nousu_kerroin=0.15,lasku_kerroin=0.08,)
        sade_orografia_vaikutuskerroin=sade_orografia_vaikutuskerroin_tulos.get()       
        kuukausi_sateet_gpu[kk] =kuukausi_sateet_gpu[kk]*sade_orografia_vaikutuskerroin

        kuukausi_lämpötilat_gpu[kk]=kuukausi_lämpötilat_gpu[kk]-kuukausi_sateet_gpu[kk]/100



    return(kuukausi_lämpötilat_gpu, kuukausi_sateet_gpu, \
    kuukausi_tuuli_suunta_gpu, kuukausi_tuuli_voima_gpu, \
    kuukausi_merivirta_x_gpu, kuukausi_merivirta_x_gpu)
...
kuukausi_lämpötilat_gpu, kuukausi_sateet_gpu, \
kuukausi_tuuli_suunta_gpu, kuukausi_tuuli_voima_gpu, \
kuukausi_merivirta_x_gpu, kuukausi_merivirta_y_gpu=laske_ilmasto_gpu(korkeus_syvyyskartta, planet_mass_me=1, planet_radius_re=1.0, planet_tilt=23.44, planet_ecc=0.013, planet_mvelp_deg=101.2, tmax_default=57.0, global_mousture_coeff=1.0)
