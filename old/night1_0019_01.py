...
# Lasketaan korkeusgradientti valmiiksi kerran ennen silmukkaa (säästää tehoa)
korkeus_gradientti_x = np.gradient(korkeus_metreinä, axis=1)
...
for kk in range(12):
    ...
    # -----------------------------------------------------------------
    # DYNAAMISET TUULET JA MERIVIRRAT (Siirretty silmukan sisään)
    # -----------------------------------------------------------------
    # Siirretään tuulijärjestelmän keskiötä (pasaatit ja länsituulet) auringon mukana.
    # lat_grid_deg - (akselin_kallistuma * 0.5) saa tuulen vaeltamaan vuodenaikojen mukaan.
    siirtynyt_lat = lat_grid_deg - (akselin_kallistuma * 0.5)
    kk_tuuli_suunta = -np.sin(np.radians(siirtynyt_lat * 3.0))

    # Lasketaan orografinen pakote dynaamisen tuulen mukaan
    kk_orografinen_pakote = korkeus_gradientti_x * kk_tuuli_suunta

    # Mallinnetaan kuukausittaiset merivirtojen anomaliat dynaamisen tuulen pohjalta
    kk_meri_anomalia = np.zeros_like(korkeuskartta)
    if np.any(maa_maski):
        virta_pakote = np.sin(np.radians(lat_grid_deg * 2.0)) * np.sign(kk_tuuli_suunta)
        kk_meri_anomalia = virta_pakote * np.exp(-etaisyys_meresta_km / 200.0) * 4.0
    
    # -----------------------------------------------------------------
    # 4.1 LÄMPÖTILA (Käyttää nyt kyseisen kuukauden merivirtoja)
    # -----------------------------------------------------------------
    ...
    kk_temp_ka += kk_meri_anomalia  # Päivitetty dynaaminen anomalia
    ...
    # Käyttää nyt kuukauden dynaamista orografista pakotetta
