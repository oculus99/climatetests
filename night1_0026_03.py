...
def laske_lampotila_vaihtelu_gpu(
    ...
    """
    Laskee lämpötilavaihtelun amplitudin (DTR) GPU:lla käyttäen CuPy-rastereita.
    """
    # 1. Potentiaalinen aurinkoenergia deklinaation mukaan (maksimi ekvaattorilla/kesällä)
    # Cos(deklinaatio) kuvaa säteilyn intensiteettiä nollaleveysasteella ilman paikkariippuvuutta
    aurinko_faktori = cp.cos(auringon_deklinaatio_rad)
    
    # 2. Mannermaisuuskerroin (etäisyys merestä kasvattaa vaihtelua logaritmisesti/lineaarisesti)
    # Lisätään 1, jotta log(1) = 0 rannikolla
    manner_faktori = cp.log1p(etaisyys_mereen_km) * 2.5
    
    # 3. Kuivuuskerroin (pieni sademäärä kasvattaa vaihtelua eksponentiaalisesti)
    # Käytetään vaimennuskerrointa, jotta suuret sademäärät saturoivat vaikutuksen nollaan
    kuivuus_faktori = cp.exp(-sademara_mm / 100.0) * 8.0
    
    # 4. Rotaation eli vuorokauden pituuden vaikutus
    # Skaalataan suhteessa Maan 24 tuntiin. Pidempi vuorokausi = enemmän aikaa lämmetä/jäähtyä.
    rotaatio_faktori = cp.sqrt(rotation_period_h / 24.0)
    
    # 5. Kiertoajan vaikutus (hitaampi kiertoaika voi mahdollistaa äärimmäisemmät vuodenajat)
    # Tässä kontekstissa orbital_period säätää deklinaation dynaamista vastetta, 
    # mutta suorana amplitudikertoimena sen vaikutus on usein käänteinen termiselle hitaudelle.
    kierto_faktori = 1.0 + (365.25 / orbital_period_days) * 0.1

    # Yhdistetään perusvaihtelu (esim. 5°C pohja) ja rasterimuuttujat
    # Kaikki laskennat tapahtuvat GPU:lla rinnakkain
    vaihtelu_rasteri = (5.0 + manner_faktori + kuivuus_faktori) * aurinko_faktori * rotaatio_faktori * kierto_faktori
...
def laske_ilmasto_gpu(
    ...
    for kk in range(12):
        ...
        (
            ...
            planet_rotation_period=planet_rotation_hours,
        ...
        merivesi_lampotila_anomalia = merivirrat_tulos[4]
        ...
        mantereisuus_lampotila_anomalia= laske_lampotila_vaihtelu_gpu(etaisyys_meresta_km ,  kuukausi_sateet_gpu[kk], deklinaatio, orbit_days, pyorinta)
        ...
        meri_pilvisyys_vaikutus_lampotilaan
        #sade_vaikutus_lampotilaan
        kuukausi_lämpötilat_gpu[kk]=perus_lampotila+ meri_pilvisyys_vaikutus_lampotilaan+lämpötila_vähetex
