...
## energy balance planet model test
...
# ============================================================
def luo_fraktaalipinta(n=77, seed=42, manner_kynnys=0.72):
    """Luo saumattoman manner/meri-pinnan 3D-pallokoordinaateissa."""
    ...
    korkeus_m = np.where(
        onko_manner,
        (korkeus - manner_kynnys) / (1.0 - manner_kynnys + 1e-12) * 4000.0,
        0.0
    )
...
def merivirrat(
    ...
    # Meridionaalinen valtamerikierto saa tässä selvästi
    # suuremman roolin kuin pelkkä lämpötilan diffuusio.
    # Lämmin vesi kuljettaa lämpöä kohti korkeampia leveysasteita.
    # Vastakkainen gradienttivaste toimii yksinkertaistettuna
    # paluuvirtana ja pitää globaalin energiabudjetin vakaana.
    v_termo_1d = (
        -0.035
        * grad_norm
    )
    ...
    for i in range(1, n - 1):
        ...
        # Kuljetusosuus:
        # Valtameri saa kuljettaa lämpöä paljon tehokkaammin
        # kuin aiemmassa versiossa. Alpha on yhden aikastepin
        # kuljetusosuus, ei suora nopeuskerroin.
        alpha = np.clip(
            0.20
            * np.abs(v_i) / 0.25,
            0.0,
...
def ratkaise_keplerin_yhtalo(M, ecc, n_iter=12):
    """Ratkaisee E yhtälöstä M = E - ecc*sin(E)."""
...
def orbitaalinen_tila(paiva_vuodesta, P_orb, ecc, perihelion_paiva=3.0):
    """Palauttaa etäisyyden AU-yksiköissä ja true-anomalian."""
    P_orb = max(float(P_orb), 1e-9)
    ecc = float(np.clip(ecc, 0.0, 0.99))
    vuoden_pituus_paivina = 365.25 * P_orb
    perihelion = perihelion_paiva * P_orb
    M = 2.0 * np.pi * (paiva_vuodesta - perihelion) / vuoden_pituus_paivina
    E = ratkaise_keplerin_yhtalo(M, ecc)
    r_AU = 1.0 - ecc * np.cos(E)
    nu = 2.0 * np.arctan2(
...
def aurinko_deklinaatio_orbitaalinen(tilt, true_anomalia, perihelion_longitude=0.0):
    """Laskee deklinaation rata-aseman ja akselikallistuksen perusteella."""
    return np.arcsin(np.clip(
        np.sin(tilt) * np.sin(true_anomalia + perihelion_longitude),
        -1.0, 1.0
    ))
...
def aja_malli(
    ...
    for kk in range(
        ...
        r_AU, true_anomalia = orbitaalinen_tila(
            paiva, P_orb, ecc, perihelion_paiva=perihelion_paiva
        )
        dekl = aurinko_deklinaatio_orbitaalinen(
            tilt,
            true_anomalia,
            perihelion_longitude=np.deg2rad(283.0)
        )
        S_paikallinen = S / (r_AU * r_AU)
        ...
        I_kk = (
            paivittainen_insolaatio_vektori(
                LAT, dekl, S_paikallinen, n_tuntia=96
            )
        )
